...
GiftsListController = RouteController.extend({
    ...
    minAge: function() {
        return parseInt(this.params.query.minAge) || 0;
    },
    maxAge: function() {
        return parseInt(this.params.query.maxAge) || 100;
    },
    ageQuery: function() {
        return { $and: [ { age: {$gte: this.minAge()} }, { age: {$lte: this.maxAge()} } ] };
    },
    findOptions: function() {
        return {sort: this.sort, limit: this.giftsLimit()};
    },
    subscriptions: function() {
        this.giftsSub = Meteor.subscribe('gifts', this.findOptions(), this.ageQuery());
    },
    gifts: function() {
        return Gifts.find(this.ageQuery(), this.findOptions());
    },
    data: function() {
...
Router.route('find/new/:giftsLimit?', {name: 'newGifts'});
Router.route('find/best/:giftsLimit?', {name: 'bestGifts'});
